...
from app.mysql_connection import cnxpool

BASE_DIR = str(Path(__file__).resolve().parent)

embedder = SentenceTransformer(BASE_DIR + "/sbert_models/Huffon_sentence-klue-roberta-base")
...
def chatbot_answer(query):
    ...
    sql = f"""SELECT `index` , `user`, `system` FROM comfort WHERE `index` IN {index_q};"""
    cursor.execute(sql)
    res = cursor.fetchall()
